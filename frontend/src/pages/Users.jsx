...
export default function Users() {
  ...
  const getRoleIcon = (role) => {
    switch (role) {
      case 'admin': return <RiShieldUserLine className="text-purple-600" />;
      case 'operador': return <RiUserSettingsLine className="text-blue-600" />;
      case 'cliente': return <RiUserLine className="text-green-600" />;
  ...
  const getRoleConfig = (role) => {
    switch (role) {
      case 'admin': return { bg: 'bg-purple-100', text: 'text-purple-800', label: 'Administrador' };
      case 'operador': return { bg: 'bg-blue-100', text: 'text-blue-800', label: 'Operador' };
      case 'cliente': return { bg: 'bg-green-100', text: 'text-green-800', label: 'Cliente' };
      default: return { bg: 'bg-gray-100', text: 'text-gray-800', label: role };
    }
  };
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="p-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Apellido</th>
                ...
                users.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${roleConfig.bg} ${roleConfig.text}`}>
                          {getRoleIcon(u.rol)}
                          {roleConfig.label}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex gap-2 justify-end">
                          ...
                          >
                            {u.is_active ? <RiCloseCircleLine /> : <RiCheckboxCircleLine />}
                          </button>
                          <button
                            onClick={() => askDelete(u)}
        ...
        {!loading && !err && users.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50">
            <p className="text-sm text-gray-700 mb-4 sm:mb-0">
              ...
              <span className="font-medium">{count}</span> usuarios
            </p>
            <div className="flex items-center gap-2">
              <button
