...
export default function ClientFormModal({ open, onClose, onSubmit, editing }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        "Error al guardar el cliente. Por favor, verifique los datos e intente nuevamente."
      );
    } finally {
      setSaving(false);
    }
  ...
  const selectedProviders = providerOptions.filter((opt) =>
    form.proveedores.includes(opt.value)
  );
