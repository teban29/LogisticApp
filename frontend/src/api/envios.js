...
export const validarUnidadParaEnvio = async (codigoBarra, clienteId) => {
  try {
    ...
    try {
      const response = await api.get(`/api/cargas/unidades/por-codigo/?codigo_barra=${codigoBarra}`);
      ...
      const unidadClienteId = unidad.cliente_id || 
                             (unidad.carga_item && unidad.carga_item.carga && unidad.carga_item.carga.cliente);
      
      if (unidadClienteId !== clienteId) {
        const clienteNombre = unidad.cliente_nombre || 
                             (unidad.carga_item && unidad.carga_item.carga && unidad.carga_item.carga.cliente_nombre) || 
      ...
      if (unidad.estado !== 'disponible') {
        return { valida: false, error: `Unidad no disponible. Estado: ${unidad.estado}` };
      }
...
const validarConEndpointGeneral = async (codigoBarra, clienteId) => {
  try {
    ...
    // Verificar cliente
    const unidadClienteId = unidad.carga_item?.carga?.cliente || unidad.cliente_id;
    if (unidadClienteId !== clienteId) {
      const clienteNombre = unidad.carga_item?.carga?.cliente_nombre || 
    ...
    return { 
      ...
      unidad: {
        ...
        carga_item: {
          producto: {
            nombre: productoNombre,
            precio_referencia: unidad.carga_item?.producto?.precio_referencia || 0
          }
        }
      }
    };
